...
def plot_image_grid(images_np, nrow =8, factor=1, interpolation='lanczos',
                    save=False, directory='~/', filename='random.png'):
...
def load(path, img_type):
    ...
    if(img_type == 'rgb'):
        ...
        raise ValueError("img_type: "+str(img_type) + " is not supported..")
    return img

def get_image(path, imsize=-1, img_type='rgb'):
    """Load an image and resize to a cpecific size. 

    Args: 
        path: path to image
        imsize: tuple or scalar with dimensions; -1 for `no resize`
        img_type: 'grayscale' or 'rgb' or ...
    """
    img = load(path, img_type)
...
def np_to_torch(img_np,add_dim_flag=True):
